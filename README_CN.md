...
Session 期间，使用 **Notion transcription** 记录讨论内容（防止遗漏）。也可以手写笔记。

Session 结束后，整理四份书面交付物：

**1. 论文 feedback。** 列出 session 中提出的所有 fixes 和建议，按人归类。谁提出的就写在谁的名字下面。发现好的 fix 时，在修改前截图留存作为 example，可以用来改进 writing guide 和 figure guide。
...
这个要求比较轻（每人大约一小时），可以在配对 session 之前或之后进行。作者应该主动去找。
